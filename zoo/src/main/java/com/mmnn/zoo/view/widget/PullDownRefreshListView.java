...
public class PullDownRefreshListView extends BaseListView {
...
//    private Animation mAnimRotate, mAnimRotateBack;
    ...
    private void preRefresh() {
        ...
        if (mCurOffsetY == 0) {
            mCurOffsetY = getContext().getResources().getDimensionPixelSize(R.dimen.margin_180);
        }
    ...
    public void setProgressPullDrawable(Drawable drawable){
        ProgressBar prog = (ProgressBar)findViewById(R.id.pull_header_indc);
        prog.setIndeterminateDrawable(drawable);
    }
...
//    }
    ...
    private void init() {
        ...
        mPullDownToRefreshText = getContext().getString(R.string.release_to_refresh);
        mReleaseText = getContext().getString(R.string.release_to_refresh);
        mTriggerRefreshThreshold = getResources().getDimensionPixelSize(R.dimen.margin_180);
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (isDown) {
                    // 如果已经处于下拉状态，记录下当前的偏移
                    TextView extHeaderTxt = (TextView) findViewById(R.id.pull_header_txt);
                    ProgressBar indicator = (ProgressBar) findViewById(R.id.pull_header_indc);
...
//                                    indicator.startAnimation(mAnimRotateBack);
    ...
    private static final class BuncingHandler extends Handler {
        private WeakReference<PullDownRefreshListView> mReference;

        public BuncingHandler(PullDownRefreshListView pullDownRefreshListView) {
            mReference = new WeakReference<>(pullDownRefreshListView);
        }

        @Override
        public void handleMessage(Message msg) {
            PullDownRefreshListView pullDownRefreshListView = mReference.get();
